...
tbl <- read_csv('C:/Users/asamson/Desktop/IAT.csv')
...
tbl_clean <- drop_na(tbl_clean, gen)

tbl_clean <- drop_na(tbl_clean, state)
...
str(tbl_clean)
factorVar <- c('gen', 'state')
tbl_clean <- mutate_at(tbl_clean, factorVar, ~factor(.))

#check if all variables that were characters are now factors 
...
tbl_clean$gender4 <- recode(tbl_clean$gen, "[2]" = 'F', "[1]" = 'M', "[5]" = 'Q', .default = "Other")
...
tbl_clean$edu7 <- recode(as.factor(tbl_clean$edu),'1' = 'No Highschool','2' = 'No Highschool', '3' = 'Some Highschool',
                         ...
                         '11' = 'Advanced Degree', '12' = 'Advanced Degree','13'='Advanced Degree','14'= 'Advanced Degree')
# mutating variables ---------------------------------------------  
# rewrite the above recoding steps so that they both occur within a single call of the mutate function
tbl_clean <- mutate(tbl_clean, gender4 = recode(gen, "[2]" = "F", "[1]" = "M", "[5]" = "Q", .default = "Other"),
...
# white men
WM <- filter(tbl_clean, gender4 == 'M' & race == 6)
print(mean(WM$bias))

# white women
WW <- filter(tbl_clean, gender4 == 'F' & race == 6)
print(mean(WW$bias))

# advanced degree holders who are men
AM <- filter(tbl_clean, gender4 == 'M' & edu7 == 'Advanced Degree')
print(mean(AM$bias))

# high school graduates who are men
HSM <- filter(tbl_clean, gender4 == 'M' & edu7 == 'Highschool Graduate')
print(mean(HSM$bias))
